...
client.defineJob({
  ...
  run: async (payload, io, ctx) => {
    const photos = await io.runTask(
      `task-1`,
      async (task) => {
        return createLargePayload(10_000); // 237781 bytes = 0.23 MB
      },
      { name: `Task 1` }
    );

    await io.runTask(`task-2`, async (task) => {}, {
      name: `Task 2`,
      params: { photos },
    });
    ...
    const morePhotos = await io.runTask(
      `task-3`,
      async (task) => {
        return createLargePayload(125_000); // 3277781 bytes = 3.13 MB
      },
      { name: `Task 3` }
    );

    await io.runTask(`task-4`, async (task) => {}, {
      name: `Task 4`,
      params: { morePhotos },
    });
  },
